...
MidiReceiverManager.prototype = {
	enable: function(portNum) {
		...
		receiver.listen((deltaTime, message) => {
			this._eventEmitter.emit(
				...
				portNum, deltaTime, message.status, message.data1, message.data2
			)
		})
	},
	...
	listen: function(portNum, callback) {
		if (!callback instanceof Function) { return }
		this._eventEmitter.on(
			MidiReceiverManager.Event.RECEIVE,
			(event, portNum, deltaTime, status, data1, data2) => {
				callback(portNum, deltaTime, status, data1, data2)
			}
		)
	},
	list: function() {
...
module.exports = new MidiReceiverManager()