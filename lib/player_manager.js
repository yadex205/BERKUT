...
const PlayerManager = function () {
    ...
    ipcMain.on(Event.DO_PLAY, (index, filepath) => { this.play(index, filepath) })
    ipcMain.on(Event.DO_PAUSE, (index) => { this.pause(index) })
    ipcMain.on(Event.DO_STOP, (index) => { this.stop(index) })
    ipcMain.on(Event.DO_DESTROY, (index) => { this.destroy(index) })
}
